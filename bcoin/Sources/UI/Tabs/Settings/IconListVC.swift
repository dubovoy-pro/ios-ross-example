...
final class IconListVC: BaseViewController {
...
extension IconListVC : UITableViewDataSource {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return operation.availableIcons.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let appIcon = operation.availableIcons[indexPath.row]
        let cell = tableView.dequeueAndRegisterCell() as AppIconOptionCell
        cell.onTap = { [weak self] in
            self?.operation.setIcon(icon: appIcon)
        }
        let isMarked = appIcon == operation.currentIcon
        cell.updateUI(appIcon: appIcon, isMarked: isMarked)
        return cell
    }
